...
load("./output/scRNA_seurat_integrated.Rdata")
```
...
write.table(markers,"./output/Cluster_markers.txt",sep="\t")
...
none detected
```{r, Dopaminergic markers, fig.height=4, fig.width=8,warning=FALSE}
...
```{r, fig.cap="Serotonergic markers", fig.height=4, fig.width=4,warning=FALSE}
...
```{r, fig.cap="Cholinergicneurons markers", fig.width=4., fig.height=4,warning=FALSE}
...
writexl::write_xlsx(gostres$result, path =  "./output/GO_termsCluster.xlsx")
...
```{r,Calling and processing reference data, include = FALSE,warning=FALSE}
...
reasons <- perCellQCFilters(stats, sub.fields = c())
...
## Mapping reference data to query data

```{r, Mapping reference data to query data,warning=FALSE}
...
```{r, Adding reduced dims to the seurat converted object,warning=FALSE}
...
```{r,fig.cap="Celltypes", fig.width=12, fig.height=4}
...
```{r,fig.cap="HPCA clusters",fig.width=8,fig.height=8}
...
# allen_reference  = readRDS("./data/data/refData/allen_cortex.rds")
# 
...
# 
# save.image("./output/Clustering_step_2.RData")


save(scRNA.sce, seurat_integrated, Cellpalette, file = "./output/scRNA_seurat_integrated.Rdata")
