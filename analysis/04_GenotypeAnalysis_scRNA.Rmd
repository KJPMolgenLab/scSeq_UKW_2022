...
g = ggplot(reslmer, aes(x=Estimate, y=-log10(p.value), col=(adj.p<0.05), text=paste("hgnc:", rownames(reslmer))))+
  geom_point()
...
if(recalc){
  ...
  Gost.reslist.by.cluster = lapply(DEX.reslist.by.cluster, function(x){gost(x, organism = "hsapiens",
                                                                            custom_bg = gbackground,
                                                                            evcodes = T)})
...
g = ggplot(df, aes(x=Estimate, y=-log10(p.value), color=(adj.p<0.05), text=paste("hgnc:", rownames(df))))+
