...
```{r}
load(file = "/files/scRNA_Qualitycheck/output/02_Clustering_scRNA_seurat_clustered.Rdata")
```
...
```{r pseudotime UMAP plots, fig.width=12, fig.height=4}
...
```{r ## 08-Trajectory Lineages individual,echo=FALSE, fig.width=8, fig.height=8}
plots=list()
for(lin in linages){
  ind = !is.na(PS[,lin])
...
```{r ## 08-Trajectory Lineages Samples,echo=FALSE, fig.width=8, fig.height=8}
...
```{r ## 08-Trajectory analysis compare linages,echo=FALSE, fig.width=8, fig.height=5}

PS_compare<-PS[,c("Sample", linages)] %>%
  group_by(Sample) %>%
  ggpubr::get_summary_stats(type = "full")

writexl::write_xlsx(PS_compare,"/files/scRNA_Qualitycheck/output/PS_comparison.xlsx")
...
writexl::write_xlsx(Comparison_PseudoTime, path = "/files/scRNA_Qualitycheck/output/shared.pseudo_comparison.xlsx")
...
writexl::write_xlsx(Comparison_PseudoTime, path =paste0( "/files/scRNA_Qualitycheck/output/",lin,"_comparison.xlsx"))
  for(i in unique(PS$SampleID)){
    for(j in unique(PS$SampleID)){
      idx = Comparison_PseudoTime$group1==i&Comparison_PseudoTime$group2==j
      if(any(idx)){
  ...
  Resmatrix[lower.tri(Resmatrix)]=t(Resmatrix[upper.tri(Resmatrix)])
  Resmatrix[Resmatrix<10e-50] = 10e-50
  pheatmap::pheatmap(-log10(Resmatrix), cluster_rows = F, cluster_cols = F, main = lin)
...
```{r ## 08-Trajectory analysis plot dist,echo=FALSE, fig.width=8, fig.height=4}
...
save(scRNA.sce, sce_sling, seurat_integrated, Cellpalette, file = "/files/scRNA_Qualitycheck/output/03_Trajectory_scRNA_seurat_sling.Rdata")
