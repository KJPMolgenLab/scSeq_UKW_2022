...
```{r}
load(file = "./output/02_Clustering_scRNA_seurat_clustered.Rdata")
```
...
```{r ## 08-Trajectory Lineages Samples,echo=FALSE, fig.width=16, fig.height=8}
...
```{r ## 08-Trajectory analysis compare linages,echo=FALSE, fig.width=8, fig.height=8}
...
for(lin in linages){
  ...
  Resmatrix[lower.tri(Resmatrix)]=t(Resmatrix[upper.tri(Resmatrix)])
  Resmatrix[Resmatrix<10e-50] = 10e-50
  pheatmaps [[lin]] <- pheatmap::pheatmap(-log10(Resmatrix), cluster_rows = F, cluster_cols = F, main = lin)[[4]]
  Tcomp [[lin]] = Comparison_PseudoTime
}


do.call(grid.arrange, pheatmaps)
...
save(scRNA.sce, sce_sling, seurat_integrated, Cellpalette, file = "./output/03_Trajectory_scRNA_seurat_sling.Rdata")
